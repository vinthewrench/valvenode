# Standalone Valve Node Design Document (v0.2)
...
- simple firmware, easy bench debug, easy field replacement
...
- **Logic Supply**: Pololu D24V22F5 5 V regulator from 12 V input
...
- **Logic rail**: Pololu D24V22F5 5 V step-down regulator for MCU and RS-485 transceiver

**Recommended components and layout for Rev A boards (16 nodes, master power switching):**

- **TVS diode**: 1.5KE15CA or P6KE15CA bidirectional across the 12 V input terminals
- **Bulk capacitance on 12 V rail**: 470 µF electrolytic capacitor (25 V or 35 V rating) + 0.1 µF ceramic capacitor, placed physically close to the VNH7100BAS motor supply pins and Pololu VIN pin
- **Pololu additional capacitance**: 47–100 µF electrolytic on VIN
- **5 V rail**: 10–47 µF electrolytic capacitor near the ATtiny4313 VCC pin
- 0.1 µF decoupling at MCU (pins 20 and 10) and RS-485 transceiver
- Separate local bulk cap on 5 V rail

**Field Cable Recommendation**: Use standard 4-conductor direct-burial irrigation sprinkler cable (18/4 acceptable for short runs; 16/4 recommended for longer runs or 16 nodes to minimize voltage drop during valve pulses).

**Note for master-side power switching (16 nodes):** The bulk capacitance handles inrush and valve pulse current when the DSP1-L2-DC12V relay turns the 12 V bus on. Allow 250–300 ms after power-on before sending commands.
...
This section lists the **complete physical pinout** for the ATtiny4313 in **PDIP-20** and **SOIC-20**. For these two packages, the **pin numbers and signal names are the same**. Only the package body changes.  
...
| 2  | PD0 | PCINT11 / RXD | 485_RX (UART_RX from RS-485 transceiver RO) |
| 3  | PD1 | PCINT12 / TXD | 485_TX (UART_TX to RS-485 transceiver DI) |
| 4  | PA1 | PCINT9 / XTAL2 | Spare |
| 5  | PA0 | PCINT8 / CLKI / XTAL1 | Spare |
| 6  | PD2 | PCINT13 / CKOUT / XCK / INT0 | 485_DE |
| 7  | PD3 | PCINT14 / INT1 | Spare |
| 8  | PD4 | PCINT15 / T0 | VNH1_EN (PWM enable – first valve) |
| 9  | PD5 | PCINT16 / OC0B / T1 | VNH2_INB (second valve) |
| 10 | GND | Ground | Ground |
| 11 | PD6 | PCINT17 / ICP1 | VNH2_EN (PWM enable – second valve) |
| 12 | PB0 | AIN0 / PCINT0 | STATUS (Status LED) |
| 13 | PB1 | AIN1 / PCINT1 | VNH2_INA (second valve) |
| 14 | PB2 | OC0A / PCINT2 | VNH1_INB (first valve) |
| 15 | PB3 | OC1A / PCINT3 | VNH1_INA (first valve) |
| 16 | PB4 | OC1B / PCINT4 | Spare (optional second LED) |
| 17 | PB5 | MOSI / DI / SDA / PCINT5 | MOSI (ISP) |
| 18 | PB6 | MISO / DO / PCINT6 | MISO (ISP) |
| 19 | PB7 | USCK / SCL / SCK / PCINT7 | SCK (ISP) |
| 20 | VCC | Supply | +5 V logic supply |

### valve node assignments
...
| RS485_DE          | PD2        | 6            | Output    | 1 = transmit, 0 = receive (DE and /RE tied) |
| VNH1_INA          | PB3        | 15           | Output    | First valve – H-bridge direction input A |
| VNH1_INB          | PB2        | 14           | Output    | First valve – H-bridge direction input B |
| VNH1_EN           | PD4        | 8            | Output    | First valve – PWM enable (H-bridge enable only) |
| VNH2_INA          | PB1        | 13           | Output    | Second valve – H-bridge direction input A |
| VNH2_INB          | PD5        | 9            | Output    | Second valve – H-bridge direction input B |
| VNH2_EN           | PD6        | 11           | Output    | Second valve – PWM enable (H-bridge enable only) |
| STATUS_LED        | PB0        | 12           | Output    | Status / ready LED |
| RESET             | PA2        | 1            | Input     | Keep for ISP programming |
| VCC               | VCC        | 20           | Power     | +5 V logic supply |
| GND               | GND        | 10           | Power     | Ground |

### Pins reserved but currently unused in Rev A (Dual Valve)

These are available on the package but are **not assigned in the current dual valve node firmware**.
...
| 5            | PA0        | Unused / Spare |
| 7            | PD3        | Unused / Spare |
| 16           | PB4        | Spare (optional second LED or other function) |
| 17           | PB5        | MOSI (ISP programming) |
| 18           | PB6        | MISO (ISP programming) |
| 19           | PB7        | SCK (ISP programming) |
...
The pin mapping above is taken from the official ATtiny2313A/4313 pinout figure.
...
### VNH7100BAS Wiring (Dual Valve Node v0.3)

**VNH1 (First Valve)**

| VNH Pin    | Connect To     | Notes |
|------------|----------------|-------|
| INA        | MCU PB3        | Direction control – first valve |
| INB        | MCU PB2        | Direction control – first valve |
| PWM        | MCU PD4        | Enable only (H-bridge enable) |
...
| VNH Pin    | Connect To     | Notes |
|------------|----------------|-------|
| INA        | MCU PB1        | Direction control – second valve |
| INB        | MCU PD5        | Direction control – second valve |
| PWM        | MCU PD6        | Enable only (H-bridge enable) |
| SEL0       | Fixed logic level | Leave simple for rev A |
...
**Field cable recommendation:** Use standard 4-conductor direct-burial irrigation sprinkler cable (18/4 or 16/4 preferred for longer runs or 16 nodes).
